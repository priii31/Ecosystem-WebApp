...
# -----------------------------
    ...
    if uploaded is None:
        ...
        if model:
            ...
            # GAUGE METER
            # -----------------------------
            st.subheader("🧭 Ecosystem Health Score")
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    'axis': {'range': [0, 100]},
                    'bar': {'color': "green" if avg_score > 70 else ("orange" if avg_score > 50 else "red")},
                    'steps': [
...
# -----------------------------
    ...
    if uploaded is None:
        ...
        trend_feature = st.selectbox("Select Feature for Trend", numeric_cols, key="trend")

        fig = px.line(
            df,
            y=trend_feature,
            title=f"{trend_feature} Trend Over Data",
            template="plotly_dark"
        )
        ...
        hist_feature = st.selectbox("Select Feature for Distribution", numeric_cols, key="hist")
        ...
        if "Temperature" in df.columns and df["Temperature"].mean() > 35:
            insights.append("🌡 High temperature detected → ecosystem stress")
        ...
        if not insights:
            insights.append("✅ Environment conditions appear stable")
